...
Server-side participant signal, App-Store/Play-Store prep, and a final README.
**Nothing from Sessions 1–5 was rebuilt** (gating gained one fallback branch).

### Server-side participant check (closes the deep-link gating gap)
- `GameSerializer` now emits **`viewer_seat`** (`"p1"` / `"p2"` / `"p1p2"` /
  `null`) and **`viewer_is_participant`** — an authoritative, request-scoped
  signal of which seat the *requesting authenticated user* owns (derived from the
  seat user FKs). Guests have no server identity → `null`.
- `computeGating` gained a 3rd-priority branch: when there's no two-account game
  and **no local seat record** but the server says the account owns one seat
  (`viewer_seat` = `"p1"`/`"p2"`), the game is gated to that seat. This closes the
  Session-5 edge case where a game opened *fresh via a deep link* (no registry
  record) defaulted to interactive. Priority is now: two-account FKs → local seat
  registry → server `viewer_seat` → default single-device. (`p1p2` / same-account
  stays ungated; the device-local "local" record still overrides the server
  signal for a hotseat game opened by its own creator.)
- Backend tests: new `ViewerSeatTest` (6 cases — p1/p2 owner, non-participant,
  guest, guest-opponent deep-link, same-account-both-seats) + a no-context
  serializer test. **Backend 176 tests green.**
- Mobile tests: 3 new gating cases (deep-link server signal, local-overrides-
  server, p1p2-not-gated). **Mobile 41 tests green.**

### App-store preparation
- **Branded icon set** generated by `scripts/generate_icons.py` — a dependency-
  free pure-Python PNG encoder + analytic anti-aliased renderer (no Pillow). Draws
  a backgammon emblem (gold/burgundy "points" hourglass + ivory checker on a felt
  vignette) matching `src/theme.js`. Regenerate with
  `python scripts/generate_icons.py`. Outputs `icon.png` (1024), `splash-icon.png`
  (transparent), the three Android adaptive layers, and `favicon.png`.
- **`app.json`** — real product name ("The Best Backgammon"), slug
  `the-best-backgammon-app`, iOS `bundleIdentifier` + `buildNumber`, Android
  `package` + `versionCode`, dark `backgroundColor`/`primaryColor`,
  `ITSAppUsesNonExemptEncryption: false`, and an `expo-splash-screen` plugin block
  (felt bg + centred logo). Installed `expo-splash-screen` (~56.0.10).
- **`eas.json`** — `development` (dev client, internal, iOS simulator), `preview`
  (internal distribution; Android APK), and `production` (store builds,
  auto-increment) profiles + a `submit` production block.

### Final README
- Rewrote root `README.md` to document **all three** packages (backend, web,
  mobile), how to run each, the EAS build/submit flow, the `ALLOWED_HOSTS`
  networking note, updated test counts (backend **176**, mobile **41**), the full
  layout, and the `viewer_seat` API addition.

### Touched files
- `backend/game/serializers.py` — `viewer_seat` / `viewer_is_participant`.
- `backend/game/tests/test_lobby.py` (`ViewerSeatTest`), `test_serializers.py`.
- `src/game/gating.js` — server-signal fallback branch + doc.
- `src/game/__tests__/gating.test.js` — 3 new cases.
- New: `app/../eas.json`, `scripts/generate_icons.py`; regenerated `assets/*`.
- `app.json` (store metadata + splash plugin), `package.json` (expo-splash-screen).
- Root `README.md`.
- Verified: backend 176, mobile 41, and `expo export --platform ios` all clean.
...
- [x] **Server-side participant signal** — `GameSerializer.viewer_seat` /
      `viewer_is_participant`; `computeGating` consumes it to close the fresh
      deep-link gating gap.
- [x] **App-store prep** — branded icons + splash (generated), `app.json` store
      metadata + bundle ids + splash plugin, `eas.json` build/submit profiles.
- [x] **Final README** — documents backend + web + mobile, EAS build/submit,
      networking, and test counts (backend 176, mobile 41).

### Session 7 (suggested) — remaining polish
...
- [ ] Bump Node ≥22.13 to clear the SDK 56 engine warning; run `eas init` to mint
      the EAS project id before the first cloud build.

### Cross-cutting / tech debt
- [x] Mobile test suite — Jest + RNTL (logic, useGame, gating, seatRegistry,
      components); **41 tests**.
- [x] App icons / splash — branded set generated via `scripts/generate_icons.py`.
