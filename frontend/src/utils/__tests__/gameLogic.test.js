...
// Mirrors backend/game/tests/test_higher_die.py — same positions, same
// expectations. When only one of the two dice can be played but either is
// individually playable, the higher one is forced.
describe('higherDieRequiredMoves', () => {
  ...
  test('no restriction from the opening position', () => {
    expect(higherDieRequiredMoves(INITIAL_BOARD, 'p1', [1, 2])).toBeNull();
  });

  test('no restriction with an empty or single-die roll', () => {
    expect(higherDieRequiredMoves(INITIAL_BOARD, 'p1', [])).toBeNull();
    expect(higherDieRequiredMoves(INITIAL_BOARD, 'p1', [3])).toBeNull();
  });
});
